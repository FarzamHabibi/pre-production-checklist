...
Then paste a [prompt](../prompts.md) followed by the file. Use `--gate` (322 items) rather
than a whole domain — it leaves room for the code, which is the part that matters.

## Context windows

DeepSeek, Kimi and GLM all offer long-context models, which is tempting: paste all 4,349
